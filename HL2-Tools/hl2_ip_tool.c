...
static int udpsock(void){
  ...
  int yes=1; if(setsockopt(fd,SOL_SOCKET,SO_REUSEADDR,&yes,sizeof yes)<0) die("SO_REUSEADDR");
  struct sockaddr_in any; memset(&any,0,sizeof any);
  any.sin_family=AF_INET; any.sin_port=htons(0); any.sin_addr.s_addr=htonl(INADDR_ANY);
...
static int recv60(int fd, uint8_t out[60], int ms, struct sockaddr_in* from){
  ...
  int r=select(fd+1,&rf,NULL,NULL,&tv); if(r<=0) return 0;
...
static int discover(int fd, struct sockaddr_in* dst){
  uint8_t msg[60]; memset(msg,0,sizeof msg); msg[0]=0xEF; msg[1]=0xFE; msg[2]=0x02;
  struct sockaddr_in b; memset(&b,0,sizeof b);
  b.sin_family=AF_INET; b.sin_port=htons(PORT_CMD); inet_pton(AF_INET,"255.255.255.255",&b.sin_addr);
  int one=1; if(setsockopt(fd,SOL_SOCKET,SO_BROADCAST,&one,sizeof one)<0) die("SO_BROADCAST");
  if(sendto(fd,msg,60,0,(struct sockaddr*)&b,sizeof b)!=(ssize_t)60) die("sendto discover");
  uint8_t buf[60]; struct sockaddr_in src;
...
static int hl2_cmd(int fd, const struct sockaddr_in* dst, uint8_t addr,
  ...
  uint8_t msg[60]; memset(msg,0,sizeof msg);
  msg[0]=0xEF; msg[1]=0xFE; msg[2]=0x05; msg[3]=0x7F; msg[4]=addr<<1;
  msg[5]=c1; msg[6]=c2; msg[7]=c3; msg[8]=c4;
  if(sendto(fd,msg,60,0,(const struct sockaddr*)dst,sizeof *dst)!=(ssize_t)60) return -1;
  uint8_t r[60]; struct sockaddr_in src; if(!recv60(fd,r,1000,&src)) return 0;
...
static int hl2_reboot_cmd_1025(int fd, const struct sockaddr_in* cmd_dst){
  uint8_t msg[60]; memset(msg,0,sizeof msg);
  msg[0]=0xEF; msg[1]=0xFE; msg[2]=0x05; msg[3]=0x7F; msg[4]=HL2_REBOOT_ADDR<<1;
  msg[5]=0x00; msg[6]=0x00; msg[7]=0x00; msg[8]=0x01;
...
int main(int argc, char** argv){
  ...
  if(set_ip){
    ...
    new_oct[0]=(uint8_t)b0; new_oct[1]=(uint8_t)b1; new_oct[2]=(uint8_t)b2; new_oct[3]=(uint8_t)b3;
  }

  int fd=udpsock();
  struct sockaddr_in dst; memset(&dst,0,sizeof dst);
  dst.sin_family=AF_INET; dst.sin_port=htons(PORT_CMD);
  if(dev_ip){
