...
void SetAnalyzer (  int disp,     // display identifier
  ...
  if (((fmax != 0.0) || (fmin != 0.0)) && ((n_pix != a->num_pixels) || (fmin != a->f_min) || (fmax != a->f_max)
      || (calset != a->cal_set) || a->cal_changed)) {
...
void CloseBuffer(int disp, int ss, int LO) {
  ...
  if ((a->IQin_index[ss][LO] += a->buff_size) >=
      a->bsize) { //REQUIRES buff_size IS A SUB-MULTIPLE OF SIZE OF INPUT SAMPLE BUFFS!
