...
#define TX_EQ_POINTS 12
#define TX_EQ_DRAW_POINTS 1024
#define TX_EQ_FMIN 10.0
#define TX_EQ_FMAX 16000.0
#define TX_EQ_GMIN -20.0
#define TX_EQ_GMAX 20.0
#define TX_EQ_PAD_LEFT 46.0
#define TX_EQ_PAD_RIGHT 14.0
#define TX_EQ_PAD_TOP 12.0
#define TX_EQ_PAD_BOTTOM 30.0
#define TX_EQ_POINT_RADIUS 5.0
...
static TX_EQ_GRAPH *active_graph = NULL;
...
static double freq_to_x(double f, double width) {
  const double plotw = width - TX_EQ_PAD_LEFT - TX_EQ_PAD_RIGHT;
  const double lo = log10(TX_EQ_FMIN);
  const double hi = log10(TX_EQ_FMAX);
  f = clampd(f, TX_EQ_FMIN, TX_EQ_FMAX);
  return TX_EQ_PAD_LEFT + (log10(f) - lo) * plotw / (hi - lo);
}
...
static double x_to_freq(double x, double width) {
  ...
  const double lo = log10(TX_EQ_FMIN);
  const double hi = log10(TX_EQ_FMAX);
  double t = (x - TX_EQ_PAD_LEFT) / plotw;
  t = clampd(t, 0.0, 1.0);
  return pow(10.0, lo + t * (hi - lo));
}

static double gain_to_y(double g, double height) {
  const double ploth = height - TX_EQ_PAD_TOP - TX_EQ_PAD_BOTTOM;
  g = clampd(g, TX_EQ_GMIN, TX_EQ_GMAX);
  return TX_EQ_PAD_TOP + (TX_EQ_GMAX - g) * ploth / (TX_EQ_GMAX - TX_EQ_GMIN);
}

static double y_to_gain(double y, double height) {
  const double ploth = height - TX_EQ_PAD_TOP - TX_EQ_PAD_BOTTOM;
  double t = (y - TX_EQ_PAD_TOP) / ploth;
  t = clampd(t, 0.0, 1.0);
  return TX_EQ_GMAX - t * (TX_EQ_GMAX - TX_EQ_GMIN);
}

static void save_curve_to_mode(TX_EQ_GRAPH *g) {
  int mode = vfo[vfo_get_tx_vfo()].mode;
  mode_settings[mode].tx_eq_curve_degree = g->tx->eq_curve_degree;
  mode_settings[mode].tx_eq_curve_r = g->tx->eq_curve_r;
  mode_settings[mode].tx_eq_curve_umethod = g->tx->eq_curve_umethod;
  for (int i = 0; i < TX_EQ_POINTS; i++) {
    mode_settings[mode].tx_eq_weight[i] = g->tx->eq_weight[i];
  }
  copy_mode_settings(mode);
}

static void apply_curve(TX_EQ_GRAPH *g) {
#ifndef WDSP1
  SetTXAEQCurve(g->tx->id, g->tx->eq_curve_degree, g->tx->eq_curve_r, g->tx->eq_curve_umethod);
  SetTXAEQWeights(g->tx->id, TX_EQ_POINTS, g->tx->eq_weight);
#endif
...
static void degree_changed_cb(GtkComboBox *combo, gpointer data) {
  ...
  g->tx->eq_curve_degree = degrees[n];
  apply_curve(g);
...
static void weights_toggled_cb(GtkToggleButton *button, gpointer data) {
  TX_EQ_GRAPH *g = data;
  g->tx->eq_curve_r = gtk_toggle_button_get_active(button) ? 1 : 0;
  apply_curve(g);
  save_curve_to_mode(g);
}
...
static gboolean draw_cb(GtkWidget *widget, cairo_t *cr, gpointer data) {
  ...
  const double x0 = TX_EQ_PAD_LEFT;
  const double x1 = width - TX_EQ_PAD_RIGHT;
  const double y0 = TX_EQ_PAD_TOP;
  const double y1 = height - TX_EQ_PAD_BOTTOM;
...
#ifndef WDSP1
  if (g->tx->eq_curve_degree >= 1) {
    double X[TX_EQ_DRAW_POINTS];
    double Y[TX_EQ_DRAW_POINTS];
    GetTXAEQDraw(g->tx->id, X, Y);
    gboolean started = FALSE;
    for (int i = 0; i < TX_EQ_DRAW_POINTS; i++) {
      /* WDSP 2.10 currently returns X as a fraction of Nyquist. */
      double f = X[i] * (0.5 * g->tx->dsp_rate);
      if (!isfinite(f) || !isfinite(Y[i]) || f < TX_EQ_FMIN || f > TX_EQ_FMAX) { continue; }
...
#endif
  {
    for (int i = 1; i <= TX_EQ_POINTS; i++) {
      double f = (g->dragging && g->selected == i) ? g->drag_freq : g->tx->eq_freq[i];
      double gain = (g->dragging && g->selected == i) ? g->drag_gain : g->tx->eq_gain[i];
      double x = freq_to_x(f, width);
  ...
  for (int i = 1; i <= TX_EQ_POINTS; i++) {
    ...
    cairo_arc(cr, x, y, TX_EQ_POINT_RADIUS, 0.0, 2.0 * G_PI);
    cairo_fill(cr);
  }
  return FALSE;
}

static int point_at(TX_EQ_GRAPH *g, double x, double y, double width, double height) {
  int best = 0;
...
static gboolean query_tooltip_cb(GtkWidget *widget, gint x, gint y, gboolean keyboard_mode,
  ...
  if (!p) { return FALSE; }
  double f = (g->dragging && g->selected == p) ? g->drag_freq : g->tx->eq_freq[p];
  double gain = (g->dragging && g->selected == p) ? g->drag_gain : g->tx->eq_gain[p];
  char text[64];
...
static gboolean button_press_cb(GtkWidget *widget, GdkEventButton *event, gpointer data) {
  ...
  g->selected = p;
  g->dragging = TRUE;
  g->drag_freq = g->tx->eq_freq[p];
  g->drag_gain = g->tx->eq_gain[p];
...
static gboolean motion_cb(GtkWidget *widget, GdkEventMotion *event, gpointer data) {
  ...
  double flo = (g->selected > 1) ? g->tx->eq_freq[g->selected - 1] + 10.0 : TX_EQ_FMIN;
  double fhi = (g->selected < TX_EQ_POINTS) ? g->tx->eq_freq[g->selected + 1] - 10.0 : TX_EQ_FMAX;
  f = clampd(f, flo, fhi);
  f = round(f / 10.0) * 10.0;
  gain = round(gain);
  g->drag_freq = f;
...
static gboolean button_release_cb(GtkWidget *widget, GdkEventButton *event, gpointer data) {
  ...
  g->tx->eq_freq[p] = g->drag_freq;
  g->tx->eq_gain[p] = g->drag_gain;
  int mode = vfo[vfo_get_tx_vfo()].mode;
  mode_settings[mode].tx_eq_freq[p] = g->drag_freq;
  mode_settings[mode].tx_eq_gain[p] = g->drag_gain;
  copy_mode_settings(mode);
  set_spin_value_silent(g->freq_spin[p], g->drag_freq);
  set_spin_value_silent(g->gain_spin[p], g->drag_gain);
  tx_set_equalizer(g->tx);
...
static gboolean scroll_cb(GtkWidget *widget, GdkEventScroll *event, gpointer data) {
  TX_EQ_GRAPH *g = data;
  if (!g->tx->eq_curve_r) { return FALSE; }
  GtkAllocation a;
  gtk_widget_get_allocation(widget, &a);
  int p = point_at(g, event->x, event->y, a.width, a.height);
  if (!p) { return FALSE; }
  ...
  g->tx->eq_weight[p - 1] = clampd(g->tx->eq_weight[p - 1] + delta, 0.1, 99.9);
  apply_curve(g);
  save_curve_to_mode(g);
  return TRUE;
}
...
GtkWidget *tx_eq_graph_create(TRANSMITTER *tx) {
  TX_EQ_GRAPH *g = g_new0(TX_EQ_GRAPH, 1);
  g->tx = tx;
  g->selected = 0;
  GtkWidget *frame = gtk_frame_new("TX EQ Curve");
  ...
  int active = 0;
  if (tx->eq_curve_degree == 1) { active = 1; }
  else if (tx->eq_curve_degree == 3) { active = 2; }
  else if (tx->eq_curve_degree == 5) { active = 3; }
  else if (tx->eq_curve_degree == 7) { active = 4; }
  ...
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(weights), tx->eq_curve_r != 0);
...
void tx_eq_graph_bind_control(int index, GtkWidget *freq_spin, GtkWidget *gain_spin) {
  if (!active_graph || index < 1 || index > TX_EQ_POINTS) { return; }
  active_graph->freq_spin[index] = freq_spin;
  active_graph->gain_spin[index] = gain_spin;
}
