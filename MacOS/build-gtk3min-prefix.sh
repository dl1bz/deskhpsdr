  ...
  hits="$(
    ...
    while read -r f; do
      if file "${f}" | grep -q "Mach-O"; then
        bad="$(otool -L "${f}" 2>/dev/null | grep -E "/opt/homebrew/opt/glib|/opt/homebrew/opt/gobject|/opt/homebrew/opt/gio|/opt/homebrew/opt/pango|/opt/homebrew/opt/cairo|/opt/homebrew/opt/harfbuzz|/opt/homebrew/opt/gdk-pixbuf|/opt/homebrew/opt/gtk\+3" || true)"
        if [ -n "${bad}" ]; then
          echo "### ${f}"
          echo "${bad}"
        fi
