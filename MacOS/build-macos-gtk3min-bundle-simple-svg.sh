...
# Bundle gdk-pixbuf external loaders, required for GTK Adwaita SVG assets.
# Keep loader .so files flat in Contents/Frameworks.
# Do NOT create Contents/Frameworks/gdk-pixbuf-2.0/... because codesign --deep
# treats nested directories in Frameworks as bundle candidates.
...
LOADER_CACHE_TEMPLATE="$APP/Contents/Resources/gdk-pixbuf-loaders.cache.template"
...
sed "s|$GTK_PREFIX/lib/gdk-pixbuf-2.0/2.10.0/loaders|@APPDIR@/Frameworks|g" "$LOADER_CACHE_SRC" > "$LOADER_CACHE_TEMPLATE"

grep -q "libpixbufloader-svg.so" "$LOADER_CACHE_TEMPLATE" || {
  echo "ERROR: SVG loader missing from bundled loaders.cache.template"
  exit 1
}
...
GDK_PIXBUF_MODULEDIR="$APPDIR/Frameworks"
GDK_PIXBUF_TEMPLATE="$APPDIR/Resources/gdk-pixbuf-loaders.cache.template"
if [ -d "$GDK_PIXBUF_MODULEDIR" ] && [ -f "$GDK_PIXBUF_TEMPLATE" ]; then
  GDK_PIXBUF_CACHE="${TMPDIR:-/tmp}/deskhpsdr-gdk-pixbuf-loaders.cache"
  awk -v appdir="$APPDIR" '{ gsub(/@APPDIR@/, appdir); print }' "$GDK_PIXBUF_TEMPLATE" > "$GDK_PIXBUF_CACHE"
