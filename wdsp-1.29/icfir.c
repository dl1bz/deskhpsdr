...
void calc_icfir(ICFIR a) {
  ...
  impulse = icfir_impulse(a->nc, a->DD, a->R, a->Pairs, a->runrate, a->cicrate, a->cutoff, a->xtype, a->xbw, 1, a->scale,
                          a->wintype);
...
double *icfir_impulse(int N, int DD, int R, int Pairs, double runrate, double cicrate, double cutoff, int xtype,
                      double xbw, int rtype, double scale, int wintype) {
  ...
  int c_samps = (int)(cutoff / runrate * N) + (N + 1) / 2 - N /
                2;  // number of unique samples within bandpass, OK for odd or even N
